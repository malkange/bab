...
from dataloader.dataset_distributed import collate_fn_BEV, Nuscenes_pt, spherical_dataset, collate_dataset_info, \
    SemKITTI_pt
...
def main():
    ...
    parser.add_argument('-re', "--resume_epoch", default=0, type=int)
    ...
    if args.resume:
        ...
        pretrained_model = torch.load(os.path.join(model_load_path, '%s_%depoch.pt' % (args.name, args.resume_epoch)),
        ...
        print('load checkpoint file', os.path.join(model_load_path, '%s_%depoch.pt' % (args.name, args.resume_epoch)))
    ...
    if datasetname == 'SemanticKitti':
        train_pt_dataset = SemKITTI_pt(os.path.join(data_path, 'dataset', 'sequences'), cfgs, split='train',
                                       return_ref=True)
        val_pt_dataset = SemKITTI_pt(os.path.join(data_path, 'dataset', 'sequences'), cfgs, split='val',
                                     return_ref=True)
    ...
    while epoch < cfgs['model']['max_epoch']:
        ...
        if epoch >= args.resume_epoch:
            print('restart from %d epoch' % epoch)
            # '''
            # validation
            if (epoch > 0):
                if args.local_rank <= 0:
                ...
                with torch.no_grad():
                    for i_iter_val, val_dict in enumerate(val_dataset_loader):
                        ...
                        for count, i_val_grid in enumerate(val_grid):
                            ...
                            panoptic_labels, center_points = get_panoptic_segmentation(instmap[count],
                                                                                       ...
                                                                                       torch.unsqueeze(center[count],
                                                                                                       0),
                                                                                       torch.unsqueeze(offset[count],
                                                                                                       0),
                                                                                       val_pt_dataset.thing_list,
                                                                                       threshold=
                                                                                       cfgs['model']['post_proc'][
                                                                                           'threshold'],
                                                                                       nms_kernel=
                                                                                       cfgs['model']['post_proc'][
                                                                                           'nms_kernel'],
                                                                                       top_k=cfgs['model']['post_proc'][
                                                                                           'top_k'], polar=True,
                                                                                       foreground_mask=for_mask)
                            panoptic_labels = panoptic_labels.cpu().detach().numpy().astype(np.int32)
                            panoptic = panoptic_labels[
                                0, val_grid[count][:, 0], val_grid[count][:, 1], val_grid[count][:, 2]]
                    ...
                    if args.local_rank != -1:
                        ...
                        if args.local_rank > 0:
                            ...
                            pickle.dump(save_dict,
                                        open(os.path.join(tmp_path, 'result_part_{}.pkl'.format(args.local_rank)),
                                             'wb'))
                        torch.distributed.barrier()
                    if args.local_rank < 1:
                        if args.local_rank == 0:
                        ...
                        for class_name, class_pq, class_sq, class_rq, class_iou in zip(unique_label_str,
                                                                                       class_all_PQ[1:-1],
                                                                                       class_all_SQ[1:-1],
                                                                                       class_all_RQ[1:-1],
                                                                                       ious[1:-1]):
                        ...
                        if not args.no_wandb:
                            wandb.log(
                                {"scores/score": {'pq': PQ * 100, 'sq': SQ * 100, 'rq': RQ * 100, 'miou': miou * 100}},
                                step=epoch)
                        ...
                        if start_training:
                            sem_l, hm_l, os_l, im_l, pix_l = np.nanmean(loss_fn.loss_dict['semantic_loss']), np.nanmean(
                                loss_fn.loss_dict['heatmap_loss']), np.nanmean(
                                loss_fn.loss_dict['offset_loss']), np.nanmean(
                    ...
                    torch.save(my_model.state_dict(),
                               os.path.join(model_save_path_root, '%s_%depoch.pt' % (args.name, epoch)))
            ...
            for i_iter, train_dict in enumerate(train_dataset_loader):
                ...
                loss = loss_fn(sem_prediction, center, offset, instmap, train_dict['voxel_label'],
                               train_dict['gt_center'],
                               train_dict['gt_offset'], train_dict['inst_map_sparse'].long(),
                               train_dict['bev_mask'].squeeze(1).long())
